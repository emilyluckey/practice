...
params:
  studentID: "410773095"
  studentName: "李靜怡"
  repoURL: "https://github.com/emilyluckey/practice/blob/master/Exercise5-410773095.Rmd"
---
...
```{r ans11}
which(transcriptData$學號 == "edfeQ7fb8") -> a
transcriptData[a,] -> b
b
```
...
totalCredits <- function(x){
  which(transcriptData$學號 == x) -> f
  transcriptData[f,] -> g
  g$及格=(g$學期成績 >= 60)
  summarise(g,
  學號 = x,
  總修習學分數 = sum(g$學分數),
  總實得學分數 = sum(g$學分數[g$及格==T]))
}
```
...
```{r ans16}
which(transcriptData$學年 == "100" & transcriptData$學期 == "1") %>%
  transcriptData[.,] -> h
levels(as.factor(h$科目名稱)) -> 通識科目總表
通識科目總表
```
...
```{r ans18}
which(transcriptData$學年 == "104" &
        transcriptData$學期 == "1" & transcriptData$科目名稱 == "ZT13") %>%
  transcriptData$系別[.] %>%
  shannon
```
